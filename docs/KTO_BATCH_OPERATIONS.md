...
`contentId` 10개를 코드 카탈로그에 고정하고, 각 장소마다 `searchKeyword2`,
`detailCommon2`, `detailImage2`를 순서대로 호출한다. 검색 결과는 `contentId`, 공식 국문명,
관광 타입이 모두 승인값과 일치해야 하며, 상세 응답에는 주소·좌표·대표 이미지와
7개 서비스 권역으로 변환 가능한 지역 코드가 있어야 한다. 운영진 승인 사진공모전
매칭을 최우선으로 사용하고 대표·상세 이미지 순으로 보충해 고유 이미지 4장을 확보해야 한다.
...
- `POST /api/v1/admin/batch-jobs`는 `KTO_DAILY_SYNC`와 `KTO_FESTIVAL_SYNC`만 대기열에 넣는다. `202 Accepted`는 실행 완료가 아니므로 반환된 `jobId`를 조회한다.
...
- 실패 또는 부분 실패 작업만 `POST /api/v1/admin/batch-jobs/{jobId}/retry`에서 `scope=FAILED_ITEMS`로 새 job을 만들 수 있다. 원본 job은 변경하지 않으며 새 job은 `parentJobId`로 연결한다.
