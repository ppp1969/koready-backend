...
지역당 상한은 50페이지다. 2026년 6월 응답 기준 제주시는 5,518건으로 28페이지,
서귀포시는 4,618건으로 24페이지가 필요하다. 상한을 늘려도 한 번에 한 페이지만
가져와 저장하므로 메모리 사용 구조와 페이지 크기 200은 바뀌지 않는다.

KTO가 일일 호출 한도를 소진해 작업이 실패하면 즉시 반복 호출하지 않는다. EB에서
`KTO_RELATED_TOUR_RESUME_SCHEDULE_ENABLED=true`를 설정하면 한국 시간 매일
00시 15분에 가장 최신 `KTO_RELATED_TOUR_SYNC` 작업을 확인한다. 최신 작업이
`FAILED` 또는 `PARTIAL_FAILED`일 때만 기존 기준월, 지역 커서, 페이지 상한,
`autoContinue` 값을 그대로 복제해 재개한다. `autoContinue=true`인 전체 연쇄
수집만 대상이며 단발성 작업은 자동 재개하지 않는다. 날짜별 고유 키로 하루 한 번만
접수하고, 최신 작업이 완료·대기·실행 중이면 과거 실패 작업을 다시 실행하지 않는다.
Render에서는 이 설정을 항상 `false`로 유지한다.
...
- 배치 작업과 항목의 생성·시작·완료·실패·수정 시각은 애플리케이션의
  UTC `Instant`를 JDBC parameter로 명시 저장한다. MySQL
  `UTC_TIMESTAMP`/`CURRENT_TIMESTAMP`와 Java 시각을 같은 `DATETIME` 열에 섞지
  않는다. 관리자 API는 저장 시 사용한 JDBC 시간대와 무관하게 같은 시각을 `Z`
  형식으로 반환해야 한다.
- 이 규칙 적용 전의 `started_at`·`updated_at`에는 Aiven UTC 서버와
  `connectionTimeZone=Asia/Seoul` 조합에서 9시간 오차가 난 기록이 있을 수 있다.
  과거 행은 어떤 쓰기 경로를 거쳤는지 열만으로 확정할 수 없어 일괄 보정하지 않고,
  배포 이후 생성되거나 상태가 바뀐 작업부터 정확한 시각을 보장한다.
- `POST /api/v1/admin/batch-jobs`는 `KTO_FULL_CATALOG_SYNC`, `KTO_EN_SYNC`, `KTO_DAILY_SYNC`, `KTO_FESTIVAL_SYNC`를 대기열에 넣는다. `KTO_FULL_CATALOG_SYNC`와 `KTO_EN_SYNC`는 `maxPages`를 생략하면 한 job에서 20페이지를 처리하고, 성공한 경우에만 다음 페이지 범위 job을 자동으로 연결한다. `KTO_FESTIVAL_SYNC`는 `autoContinue=true`일 때만 같은 `eventStartDate`와 `maxPages`로 다음 페이지 범위를 연결한다. `202 Accepted`는 실행 완료가 아니므로 반환된 `jobId`를 조회한다.
...
- 일일 동기화는 `startPage`(기본 1, 최대 100000)와 `maxPages`(기본 1, 최대 20)를 받는다. 축제 수집은 같은 범위에 `eventStartDate`와 `autoContinue`(기본 `false`)를 추가로 받는다. 운영 전체 수집은 먼저 `autoContinue=false`의 작은 범위를 확인한 뒤 `true`로 전환한다.
...
- 작업자는 기본 30초마다 대기열에서 한 건을 claim한다. 서버 시작 시 실행 중이었다가 중단된 job은 `FAILED`로 바꾸고 슬롯을 해제한다. 운영자는 원인을 확인한 뒤 retry한다.
- 전체 장소 페이지 호출은 네트워크 오류, HTTP 429·5xx, KTO 요청 제한 코드 `22`에 한해 최대 4회 재시도한다. 재시도 간격은 1초, 2초, 4초이며 파싱 오류나 인증·요청 오류는 즉시 실패시킨다.
- KTO는 마지막 페이지의 `numOfRows`를 요청 크기보다 작은 실제 잔여 건수로 반환할 수 있다. 전체 페이지 수 계산에는 응답의 축소값이 아니라 요청한 페이지 크기를 사용해야 하며, 그렇지 않으면 존재하지 않는 다음 페이지를 요청하게 된다.
- 연관 관광지 API는 결과가 없는 지역에 `resultCode=0000`, `totalCount=0`,
  빈 `items`, `numOfRows=0`을 반환한다. 이 조합은 정상 빈 페이지로 처리하고
  클라이언트의 요청 페이지 크기로 정규화한다. 데이터가 있거나 총건수가 양수인데
  `numOfRows=0`인 응답은 계약 위반으로 계속 실패시킨다.
- 최종 실패 시 애플리케이션 로그에는 job ID, job type, 예외 클래스만 남긴다. KTO 키, 요청 URL, 원문 응답과 임의 예외 메시지는 기록하지 않는다.
