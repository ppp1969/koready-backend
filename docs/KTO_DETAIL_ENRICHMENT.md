...
- MVP 공개 API는 `show_flag=true`만으로 장소를 공개하지 않는다. 영문 현지화에
  `KTO_EN` 또는 `MANUAL_EDITED` 출처의 비어 있지 않은 제목이 함께 있어야 한다.
  `AI_TRANSLATED`와 영문 매칭이 없는 장소는 공개 목록, 검색, 추천, 저장, Buddy,
  Hori Tip, 온보딩 후보 게시에서 제외한다.
- 영문 매칭 장소를 먼저 화면에 채우기 위해 상세 수집 대상은 신뢰 영문 매칭 장소를
  우선하고, 같은 우선순위 안에서는 장소 ID 순서로 처리한다. 매일 새 체인이 ID 0부터
  시작하므로 매칭 장소를 모두 처리한 뒤 나머지 전국 장소 수집도 계속된다.
...
영문 `overview`가 없으면 한국어 설명이나 AI 생성문으로 대체하지 않고 `null`을 반환한다.
프론트는 이때 소개 영역을 숨긴다.
