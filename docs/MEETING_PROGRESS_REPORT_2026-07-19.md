...
- AWS S3 실제 배포와 원본·공모전 증빙 파일의 임시 다운로드 주소 발급
- 관리자의 수동 배치 실행·재시도, 증빙 묶음 생성, 상세 감사 로그 조회

**권장 회의 결론:** 현재 구현된 화면 계약을 1차 동결하고 프론트 연동을 시작한다. 동시에 로그인·약관을 다음 백엔드 우선순위로 올리며, Route 화면과 Hori Tip 노출 위치는 디자인 확인 후 개발한다. AWS 작업은 결제 확인 전까지 코드 밑바탕만 유지한다.
...
- 향후 운영: AWS Elastic Beanstalk는 약 한 달 뒤 재검토, S3는 KTO 원본·증빙 보관용 밑바탕만 준비
...
| 공모전 증빙 묶음 | 0/4 | S3 실제 배포 이후 구현 | C |
...
| 명세/C | snapshot 다운로드 `POST /admin/open-api/snapshots/{snapshotId}/download-url` | snapshot ID | 짧게 유효한 private 다운로드 URL | AWS S3 실제 배포 후 구현 |
...
- AWS S3 복구, 권한, 비용 알림은 결제 확인 후 실제 계정에 배포해야 검증된다.
...
1. AWS 결제 확인 후 서울 리전에 private S3 CloudFormation 배포
2. local 저장과 S3 저장의 같은 파일·해시 결과 검증
3. snapshot 임시 다운로드, 공모전 증빙 묶음 생성·다운로드 구현
4. 관리자 감사 로그를 실제 로그인 사용자 정보와 연결
5. 수동 배치·재시도에 동시 실행, 비용, 최대 범위 보호장치 추가
...
| AWS 시점 | 결제 확인 후 S3부터, EB는 약 한 달 뒤 재검토 | 증빙 다운로드와 운영 배포 지연 | 미기입 |
