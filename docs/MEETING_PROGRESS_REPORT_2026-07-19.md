...
> 기준 시각: 2026-07-19 23:39 KST
> 대상: PM, 디자이너, 프론트엔드, 백엔드
> 목적: 현재까지 만든 범위와 신뢰 수준을 공유하고, 다음 개발 전에 제품·화면 결정을 한 번에 확인한다.

## 1. 회의에서 먼저 볼 결론

KoReady 백엔드는 **관광지 데이터 수집·가공, 온보딩, 위치, 홈, 추천, 장소, Buddy 프로필·쪽지·신고, 관리자 큐레이션**까지 실제 코드와 DB로 구현되어 있다. 현재 Swagger에 정의된 API는 71개이며, 그중 **62개는 구현**, **9개는 앞으로 만들 계약만 정의**되어 있다.

단, 62/71이라는 수치인 **87.3%는 API 개수 기준**이다. 화면 난이도, 외부 서비스 연동, 운영 준비도를 반영한 전체 프로젝트 완성률로 해석하면 안 된다.
...
| 외부 API 운영 조회 | 6/6 | 호출·원본 메타데이터 조회, 5분 snapshot 다운로드 | A/B |
| 배치·동기화 관리 | 8/8 | 조회·활성화·cursor 초기화, KTO 수동 실행·재시도 | B |
| 공모전 증빙 묶음 | 4/4 | 단일 작업자 ZIP 생성, manifest·SHA-256, 상태 조회, S3 signed URL | A/B |
| 데이터 품질·감사 | 1/2 | 품질 요약 구현, 상세 감사 목록은 명세 | A/C |
| **합계** | **62/71** | API 개수 기준 87.3% | 전체 완성률 아님 |
...
| 구현/A/B | snapshot 상세 `GET /admin/open-api/snapshots/{snapshotId}` | snapshot ID | 해시, 크기, 압축, 저장 위치, 보관 정책, 불변·다운로드 가능 여부 | S3 활성·미만료 KTO만 `downloadable=true` |
| 구현/A/B | snapshot 다운로드 `POST /admin/open-api/snapshots/{snapshotId}/download-url` | snapshot ID | 기본 5분 private GET URL, 파일명, 원문·gzip SHA-256 | URL·AWS 자격증명은 감사 로그에 저장하지 않음 |
| 구현/B | 배치 목록 `GET /admin/batch-jobs` | 작업 종류·상태·실행 출처·시간·cursor·size | 작업 요약 목록 | Render 500MB 운영 한도 주의 |
| 구현/B | 수동 배치 실행 `POST /admin/batch-jobs` | `KTO_DAILY_SYNC` 또는 `KTO_FESTIVAL_SYNC`, 페이지 범위, 축제 기준일, 사유 | `PENDING` job과 상태 조회용 ID | 한 번에 하나만 접수. 202는 완료가 아니라 대기열 등록 |
...
| 구현/B | 배치 재시도 `POST /admin/batch-jobs/{jobId}/retry` | `FAILED_ITEMS`, 사유 | 원본을 보존한 새 retry job | `FAILED`·`PARTIAL_FAILED`만 가능, 전역 한 건 제한 적용 |
...
- Aiven 원격 DB에 여러 건을 한 번에 보내도록 staging 설정을 조정한 뒤, 신규 KTO 200건의 애플리케이션 처리 시간이 약 104초에서 약 7초로 줄었다. Gradle 실행 전체도 약 119초에서 22.4초로 줄었고 기존 축제 203건은 그대로 유지됐다. 비교 실행의 날짜와 snapshot 저장 방식이 달라 정밀 부하 시험 결과로 보기는 어렵지만, S3 직접 저장이 약 1.15초였다는 별도 측정상 DB 왕복 최적화 효과는 충분히 확인됐다. 장시간 반복 실행의 최대 메모리는 추가 검증이 필요하다.
...
1. 공모전 증빙 묶음 생성·목록·상세·다운로드 구현
2. 관리자 감사 로그를 실제 로그인 사용자 정보와 연결
3. Render 500MB 기준 수동 배치의 장시간 반복 메모리와 실패 복구 안정성 측정
4. 실제 KTO secret을 가진 staging 환경에서 수동 배치 한 페이지 스모크 실행
