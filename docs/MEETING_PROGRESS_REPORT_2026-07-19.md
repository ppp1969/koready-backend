...
- AWS S3의 비공개 설정, 최소 권한, 조건부 쓰기와 USD 5 월간 비용 알림은 실제 계정에서 검증했다. 장기 보존 비용, 대량 객체 복구와 장애 훈련은 아직 검증하지 않았다.
...
1. local 저장과 S3 저장의 같은 KTO 파일·해시 결과를 실제 수집 1회로 검증
