...
| 한국에 온 목적 | EXCHANGE_STUDENT, LANGUAGE_STUDY, SHORT_TRIP, DEGREE_PROGRAM, INTERN_WORK, WORKING_HOLIDAY, ETC |
...
- 온보딩 선호 관광 유형은 최대 4개를 초과할 수 없다. 1개 이상 선택을 권장하되, 기획상 skip을 허용할지 별도 결정한다.
...
- 축제는 한국 시간 기준 시작일이 오늘부터 6개월 이내인 경우에만 노출한다.
- 이미 시작한 축제는 종료일이 지나지 않았다면 노출할 수 있고, 종료일이 지난 축제는 노출하지 않는다.
