...
- 승인 장소 상세 갤러리는 서로 다른 이미지 4장을 반환한다. 운영진이 장소 연결을 확인한
  관광공모전 수상 사진을 가장 먼저 두고, 기존 `firstimage`, KTO `detailImage2` 순으로 보충한다.
  수상 사진 API는 TourAPI 관광지 contentId를 직접 제공하지 않으므로 제목 유사도만으로 자동
  연결하지 않고 운영진 검수 매핑을 거친다. 고유 이미지가 4장 미만이면 후보 세트를 발행하지 않는다.
