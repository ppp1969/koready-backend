...
- 장소 상세 갤러리는 최대 3장을 반환한다. 기본 순서는 KTO `detailImage2`, 부족하면 기존
  `firstimage`이며, 운영진이 장소 연결을 확인한 관광공모전 수상 사진은 이보다 높은 우선순위로
  저장한다. 수상 사진 API는 TourAPI 관광지 contentId를 직접 제공하지 않으므로 제목 유사도만으로
  자동 연결하지 않고 운영진 검수 매핑을 거친다.
