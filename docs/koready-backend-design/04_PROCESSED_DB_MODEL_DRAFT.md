...
이 문서는 migration 확정본이 아니라, Figma 기반 기능 구현을 위해 필요한 데이터 모델 초안이다. 실제 테이블/컬럼명은 개발 컨벤션에 맞게 조정한다.
...
## 2.1 user_onboarding_profiles

| 컬럼 | 설명 |
|---|---|
| id | PK |
| user_id | FK |
| travel_purpose | 한국에 온 목적 |
| completed_at | 온보딩 완료 시각 |

## 2.2 user_travel_styles
...
## 2.4 onboarding_preference_candidate_sets
...
## 2.5 onboarding_preference_candidates
...
## 2.6 user_onboarding_place_selections
...
| recommendable_from | `start_date - 6개월`, 축제 노출 시작일 |
| recommendable_until | `end_date`, 축제 노출 종료일 |
| date_validation_status | VALID/MISSING/INVALID |

`Asia/Seoul` 기준 현재 날짜가 `recommendable_from`보다 이르거나 `recommendable_until`보다 늦으면 축제를 노출하지 않는다. 경계 날짜는 포함한다.

## 4.5 place_intro_details
...
| place_id | FK |
| month | 1~12 |
| reason_type | EVENT_PERIOD/AI_SEASON/MANUAL |
...
- suppress 기간은 `CARD_SERVED` 시각부터 30일로 고정한다.
...
4. user_onboarding_profiles
5. user_travel_styles
6. user_locations
7. onboarding_preference_candidate_sets / onboarding_preference_candidates
8. user_onboarding_place_selections
9. user_preference_tags
10. service_regions / administrative_regions
11. tour_api_category_codes / tour_api_lcls_codes
12. places
13. place_localizations / place_source_records / place_source_matches
14. place_images
15. place_tags / tags / place_style_mappings
16. place_event_periods
17. user_saved_places
18. user_place_events
19. recommendation_decks / recommendation_deck_items
20. user_place_recommendation_states
21. open_api_call_logs / open_api_raw_snapshots
22. tour_api_sync_cursors
23. batch_jobs / batch_job_items
24. user_roles / admin_audit_logs
25. evidence_bundles / evidence_bundle_items
