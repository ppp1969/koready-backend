...
| 컬럼 | 설명 |
|---|---|
| id | 내부 숫자 PK. API에는 노출하지 않음 |
| public_id | `thread_` prefix의 opaque API thread ID, case-sensitive unique |
| place_id | 관련 여행지 FK |
...
`idempotency_key`는 대소문자를 구분하며, MVP에서는 자동 만료시키지 않는다. 삭제·보관 기간은 실제
전송량을 확인한 뒤 운영 정책으로 추가한다.

현재 구현은 Flyway `V16__add_buddy_messages.sql`을 기준으로 하며 메시지 본문, 멱등성 예약,
스레드의 마지막 메시지 갱신은 하나의 DB transaction에서 처리한다.
...
MVP 안전장치다. 차단은 `buddy_blocks`에서 방향성을 가진 사용자 관계와 최초 차단 시각을
보존한다. 신고는 Flyway `V17__add_buddy_reports.sql`을 기준으로 다음 정보를 저장한다.

| 컬럼 | 설명 |
|---|---|
| reporter_user_id | 신고한 활성 사용자 FK |
| target_type | PROFILE / MESSAGE |
| target_profile_id | 신고 대상 프로필. MESSAGE이면 발신자 프로필 |
| target_message_id | MESSAGE 신고 원본 FK, PROFILE이면 null |
| reason | trim 후 1~500자 신고 사유 |
| status | RECEIVED / REVIEWING / RESOLVED / REJECTED |
| idempotency_key / request_hash | 사용자별 중복 제출 방지와 충돌 검증 |
| created_at / updated_at | 접수 및 상태 변경 시각 |

`reporter_user_id + idempotency_key`는 unique다. 메시지 본문은 신고 행에 복제하지 않고 원본
메시지 FK를 운영 증빙으로 유지한다. 신고 접수는 차단이나 자동 제재를 만들지 않는다.
