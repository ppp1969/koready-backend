...
구현 상태는 네 endpoint 모두 `IMPLEMENTED`다. 모든 endpoint는 사용자 인증이 필요하고,
경로의 위치 ID가 다른 사용자 소유이거나 삭제된 경우 존재 여부를 구분해 노출하지 않고
`404 USER_LOCATION_NOT_FOUND`를 반환한다.

| Method | URI | 설명 |
|---|---|---|
| GET | `/api/v1/users/me/locations` | 기본 위치 우선, 나머지는 최신순인 활성 위치 목록 |
| POST | `/api/v1/users/me/locations` | 검색 결과를 내 위치로 저장 |
...
서버는 token의 서명과 만료를 검증해 검색 당시 주소·좌표·권역을 저장한다. 클라이언트가
주소, 좌표, provider ID 또는 권역 코드를 위치 저장 요청에서 다시 보내지 않는다. 저장
요청에서는 Kakao를 다시 호출하지 않으므로 검색 이후 짧은 provider 장애가 저장 결과를
바꾸지 않는다.

첫 활성 위치는 `setDefault=false`여도 기본 위치가 된다. 이미 기본 위치가 있는 경우에는
`setDefault=true`일 때만 새 위치로 변경한다. 생성 성공은 `201 USER_LOCATION_CREATED`다.
...
```json
{
  ...
  "displayName": "성신여자대학교",
  "customLabel": "학교",
  "roadAddress": "서울특별시 성북구 보문로34다길 2",
...
- `customLabel`은 선택값이며 공백 제거 후 최대 30자다. 빈 문자열은 null로 저장한다.
- token이 만료되면 `410 LOCATION_SEARCH_RESULT_EXPIRED`이고 프론트는 같은 검색어로 재검색한다.
- token 형식이 잘못됐거나 서명이 변조되면 `422 LOCATION_SEARCH_RESULT_INVALID`다.
- 목록은 `200 USER_LOCATION_LIST_OK`이며 삭제된 위치와 상세 주소·좌표가 없는 이전 개발 행은 제외한다.
- 기본 위치 변경은 `200 USER_LOCATION_DEFAULT_UPDATED`다. 사용자 행 잠금으로 동시 변경을 직렬화한다.
- 삭제 성공은 body 없는 204다. 기본 위치 삭제 시 남은 위치 중 가장 최근 생성된 위치를 자동 지정하고, 남은 위치가 없으면 기본 위치를 null로 둔다.
- 위치 상세값은 추천과 경로 계산용 개인정보이므로 로그와 오류 응답에 기록하지 않는다.
...
- 같은 두 프로필과 장소 조합은 스레드 하나만 유지하고, 이후 첫 쪽지 요청도 그 스레드에 새 메시지를 추가한다.
- 자기 자신, 양방향 차단 관계, 비공개 또는 수신 거부 프로필에는 전송할 수 없다.
- 타인의 스레드와 존재하지 않는 스레드는 모두 `404 MESSAGE_THREAD_NOT_FOUND`로 처리해 참여 관계를 노출하지 않는다.
- 쪽지함은 `updatedAt DESC, 내부 threadId DESC`로 정렬하고 cursor를 그대로 다음 요청에 전달한다. 최신 메시지 미리보기는 공백 정리 후 최대 100자다.
- 스레드 조회 응답은 화면 표시 순서인 `messageId ASC`다. 다음 cursor는 더 오래된 메시지를 가리키므로 새 페이지를 기존 배열 앞에 붙인다.
...
| `USER_LOCATION_NOT_FOUND` | 내 활성 위치가 아니거나 이미 삭제됨 |
| `LOCATION_SEARCH_RESULT_EXPIRED` | 위치 검색 token 10분 만료 |
| `LOCATION_SEARCH_RESULT_INVALID` | 위치 검색 token 형식 또는 서명 오류 |
