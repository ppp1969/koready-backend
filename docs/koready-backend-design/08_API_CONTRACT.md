...
`year/month`가 기본 조회 범위다. `dateFilterType`을 함께 보내면 선택한 연월 범위와 날짜 필터 범위의 교집합만 반환한다. `CUSTOM`일 때는 `customStartDate/customEndDate`가 모두 필수다. `sort=RECOMMENDED`에서는 `ONGOING`, `UPCOMING`, `ENDED` 순의 상태 우선순위를 기본 점수보다 먼저 적용한다.
