...
> - 문서 상태: PM 검토 및 개발 착수 승인 요청
> - 기준일: 2026-07-14 (Asia/Seoul)
> - 제품 기준: KOREADY Figma `UI` 페이지와 2026-07-13 확정 정책
...
Koready는 한국을 방문하거나 체류하는 외국인을 위한 로컬 여행 준비 및 추천 서비스다.

- 교환학생, 어학연수생, 학위과정 학생, 인턴/근로자, 워킹홀리데이 참가자, 단기 여행자를 포함한다.
...
| 온보딩 단계 | 방문 목적과 위치가 모두 `01` | 화면 단계 번호 정리. 서버 상태값으로 사용하지 않음 |
...
3. 언어는 `KO` 또는 `EN`이다.
4. 방문 목적 하나를 선택한다.
5. 위치 검색어는 Koready 백엔드로 보내며 백엔드가 Kakao Local API를 호출한다.
6. 검색 결과에 포함된 10분 유효 `searchResultToken`으로 위치를 저장한다.
7. 여행 스타일을 중복 없이 1~4개 선택한다.
8. 현재 발행된 관리자 큐레이션 10개 중 1~3개 장소를 선택한다.
9. 서버가 위치 소유권, 스타일 개수, 후보 세트 버전, 선택 장소 포함 여부를 한 transaction에서 검증하고 온보딩을 완료한다.
10. 같은 완료 요청이 재전송돼도 동일 상태를 반환한다.

### 4.2 홈과 탐색

- 홈은 기본 위치, 사용자 언어, 현재 월을 기준으로 필요한 요약 데이터를 한 번에 반환한다.
- 월별 추천은 행사 기간이 유효한 축제와 월별 추천 feature를 조합한다.
...
| 언어 | `KO`, `EN` |
| 소셜 로그인 | `GOOGLE`, `APPLE` |
| 방문 목적 | `EXCHANGE_STUDENT`, `LANGUAGE_STUDY`, `SHORT_TRIP`, `DEGREE_PROGRAM`, `INTERN_WORK`, `WORKING_HOLIDAY`, `ETC` |
...
- 축제는 `startDate - 6개월`부터 `endDate`까지 노출 가능하다.
- 경계 날짜는 포함하고 시간대는 `Asia/Seoul`이다.
...
| 월별 축제 | 가능 | 행사 시작/종료일 가공, 누락/오류 제외 |
...
| 온보딩 profile | `user_id`당 1개 | DB unique + 단일 transaction | 완료 재요청 결과 동일 |
...
| 추천 상태 | `(user_id, place_id)` unique | upsert + `suppress_until` | 노출 후 30일 새 덱 제외 |
...
- 서비스 권역, 장소 master, localization, image, event, tag/style
...
각 항목에 `승인`, `수정`, `보류` 중 하나와 수정 내용을 기록한다.

| ID | 승인 요청 내용 | 권장안 | PM 판정/메모 |
|---|---|---|---|
| D-01 | 대상 사용자와 언어 | 방한·체류 외국인, KO/EN | `[ ] 승인 [ ] 수정 [ ] 보류` |
...
| D-07 | 온보딩 스타일 선택 | 중복 없이 1~4개 필수 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-08 | 선호 장소 | 관리자 발행 10개 중 1~3개, 발행본 불변 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-09 | 월별 축제 | 시작 6개월 전부터 종료일까지 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-10 | `NEARBY` 의미 | 거리 반경이 아니라 같은 7권역 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-11 | 추천 재노출 | `CARD_SERVED` 후 30일, 후보 부족 시에도 단축 안 함 | `[ ] 승인 [ ] 수정 [ ] 보류` |
...
| D-14 | Hori Tip | 운영진 작성만 허용, route 조회 시점에 동적 조합 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-15 | Buddy Connect | 공개 프로필, 장소별 mates, 비실시간 1~1,000자 쪽지, 차단/신고 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-16 | MVP 제외 | GPS, 실시간 채팅, 체크리스트, 오디오, 숙박 추천 제외; KTX 가이드는 정적 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-17 | 데이터 보관 | KTO snapshot만 조건부 장기 보관, TMAP/Kakao/OAuth 민감 원문 금지 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-18 | 유일성 | 7장의 unique/transaction/token/idempotency 정책을 migration 기준으로 채택 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-19 | 인프라 | 현재 Render+Aiven, 약 한 달 뒤 AWS EB 재검토 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-20 | API 범위 | 현재 59 path/71 operation을 MVP 계약 기준선으로 채택 | `[ ] 승인 [ ] 수정 [ ] 보류` |
| D-21 | Figma 보정 | 3.4의 GPS 버튼, 180분, 1,000자, 권역/단계 표기를 수정 | `[ ] 승인 [ ] 수정 [ ] 보류` |
...
| `OnboardingStep` | `TRAVEL_PURPOSE`, `LOCATION`, `TRAVEL_STYLES`, `PREFERENCE_PLACES`, `COMPLETED` |
