...
if (-not $env:KTO_SNAPSHOT_DIRECTORY) {
    $env:KTO_SNAPSHOT_DIRECTORY = Join-Path $HOME '.koready\kto-snapshots'
}
...
Push-Location $repositoryRoot
try {
    Write-Host "Starting bounded KTO festival import: profile=$Profile, eventStartDate=$EventStartDate, startPage=$StartPage, maxPages=$MaxPages"
    & .\gradlew.bat importKtoFestivals `
        "-Pprofile=$Profile" `
        "-PeventStartDate=$EventStartDate" `
        "-PstartPage=$StartPage" `
        "-PmaxPages=$MaxPages" `
        --no-daemon
    if ($LASTEXITCODE -ne 0) {
        throw 'KTO festival import failed.'
    }
}
finally {
    Pop-Location
