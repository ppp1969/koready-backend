...
@ConfigurationProperties(prefix = "koready.kto.related-tour-api")
public record KtoRelatedTourApiProperties(
	URI baseUrl,
	String serviceKey,
	...
	private static final String REQUIRED_PATH_SUFFIX =
		"/B551011/TarRlteTarService1";

	public KtoRelatedTourApiProperties {
		if (baseUrl == null
			...
			throw new IllegalArgumentException(
				"KTO related tour API base URL is not approved");
		}
		if (maxResponseBytes < 1
			|| maxResponseBytes > MAX_ALLOWED_RESPONSE_BYTES) {
			throw new IllegalArgumentException(
				"KTO related tour response limit is invalid");
		}
		if (connectTimeout == null || connectTimeout.isZero()
			...
			throw new IllegalArgumentException(
				"KTO related tour timeouts must be positive");
		}
		if (mobileOs == null || mobileOs.isBlank()
			|| mobileApp == null || mobileApp.isBlank()) {
			throw new IllegalArgumentException(
				"KTO related tour mobile identification is required");
		}
	}
}
